  ...
  queueManager.wallet.wallet
    ...
    .then((chainInfo) => {
      ...
      if (chainInfo.chainid.magic) {
        ...
        defaultContext.networkName = `${
          chainInfo.chainid.public ? "public" : "private"
        } ${chainInfo.chainid.mainnet ? "mainnet" : "non-mainnet"}`;
      }
    });
    ...
    request.post(
      ...
      (error, response, body) => {
        ...
        res.render("request-success", {
          ...
          aergoscanUrl: getAergoscanUrl(
            `account/${req.body.address}?keyword=${req.body.address}`
          ),
          next,
          hasNext: !!next,
        });
